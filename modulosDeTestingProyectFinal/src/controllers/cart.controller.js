...
export const getCarts = async (req, res) => {
  try {
    ...
    return res.status(200).json({ message: "Existing carts in data base: ", carts });
  } catch (err) {
...
export const getCartById = async (req, res) => {
  try {
    ...
    if (!cart) {
      ...
      return res.status(200).json({ message: "Cart info: ", cart });
    }
  } catch (err) {
    req.logger.error(err);
    return res
  ...
  try {
    ...
    if (!validationUtils.validateProductBody(updatedProductBody)) {
      return res.status(400).json({
        message:
          "Only title, description, code, price, status, stock and category can be modified.",
      });
    }
    const cart = await CartService.getCartById(cid);
    if (!cart) {
...
export const deleteCartById = async (req, res) => {
  try {
    ...
    if (!cart) {
      ...
      return res
        .status(200)
        .json({ message: "Cart deleted.", deletedCart });
    }
  } catch (err) {
    req.logger.error(err);
    return res
