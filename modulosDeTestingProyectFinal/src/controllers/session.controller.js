...
export const recoverUser = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "There was an error recovering user." });
  }
};
export const resetPassword = async (req, res) => {
  try {
    ...
    if (!isValidComparePwd) {
      ...
      return res
        .status(400)
        .json({ message: "Password cannot be the same as the existing one." });
    }
  } catch (err) {
    req.logger.err(err);
    return res
      .status(500)
      .json({ message: "There was an error resetting password." });
  }
};
